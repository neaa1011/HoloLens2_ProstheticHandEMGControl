...
public class GestureData
{
    ...
    public Dictionary<string, JointData> Joints { get; set; }
}
...
public class LeftHandController : MonoBehaviour
{
    ...
    private Dictionary<string, Quaternion> RotSpreadData;
    ...
    private void Start()
    {
        ...
        if (isClassifying && (isTracked == TrackedHandside.Right))
        {
...
#if UNITY_EDITOR
            LoadData(CustomGestureID.Fist, "./Assets/Scripts/data/LeftHand/fist.txt");
            LoadData(CustomGestureID.Pinch, "./Assets/Scripts/data/LeftHand/pinch.txt");
            LoadData(CustomGestureID.Spread, "./Assets/Scripts/data/LeftHand/spread.txt");
            LoadData(CustomGestureID.ThumbsUp, "./Assets/Scripts/data/LeftHand/thumbup.txt");
#elif !UNITY_EDITOR && UNITY_METRO
            nextGestureID = CustomGestureID.Fist;
            LoadData(CustomGestureID.Fist, "LeftHand/fist");
            nextGestureID = CustomGestureID.Pinch;
            LoadData(CustomGestureID.Pinch, "LeftHand/pinch");
            nextGestureID = CustomGestureID.Spread;
            LoadData(CustomGestureID.Spread, "LeftHand/spread");
            nextGestureID = CustomGestureID.ThumbsUp;
            LoadData(CustomGestureID.ThumbsUp, "LeftHand/thumbup");
            nextGestureID = CustomGestureID.None;
#endif
            if (isEMG)
            {
                Debug.Log("*********** Set Invoke Repeating GetEMGGstures() **********");
                InvokeRepeating("GetEMGGestureLabel", 2f, 0.25f); //Start after 2 sec, called every 4 sec
            }
            else
            {
                Debug.Log("*********** Set Invoke Repeating GetVisualGestureLabel() **********");
                InvokeRepeating("GetVisualGestureLabel", 2f, 0.25f);
            }           
        }
    }
    ...
    private void GetEMGGestureLabel()
    {
        ...
        if (isTracked == TrackedHandside.Right)
        {
            ...
            Debug.Log($"********** GetEMGGestureLabel() Label from file is {GestureID}**********");

            nextGestureID = (CustomGestureID)Enum.Parse(typeof(CustomGestureID), GestureID);
        }
    }

    private void GetVisualGestureLabel()
    {
        if (isTracked == TrackedHandside.Right)
        {
